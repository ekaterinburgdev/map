...
export function DTPFilter() {
  ...
  const [severityState, dispatchSeverity] = useReducer(dtpSeverityReducer, dtpSeverityInitalState);
  ...
  useEffect(() => {
    dtp.getSeverityFilters().then((severityFilters: SeverityCountEntries) => {
      const sortedSeverityCount = severityFilters.sort(([, countA], [, countB]) => countB - countA);
