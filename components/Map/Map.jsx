...
function Map() {
  ...
  reqHouses.open("GET", "http://51.178.191.76:1337/api/house?populate=geometry");
  reqHouses.send();

  return (
    <span>
      <button className="bbb" onClick={() => setIsHousesVisible(!isHousesVisible)}>Click me!</button>
      <MapContainer
      ...
      >
        ...
        {isHousesVisible ? <HousesMarkers housesPositionsData={housesPositionsData}/> : null}
        <OknMarkers oknPositionsData={oknPositionsData}></OknMarkers>
      </MapContainer>
    </span>
  );
}
...
function OknMarkers(oknPositionsData){
  const markers = [];
  if (!oknPositionsData.oknPositionsData || !oknPositionsData.oknPositionsData.data)
    return;
...
function HousesMarkers(housesPositionsData){
  const markers = [];
  if (!housesPositionsData.housesPositionsData || !housesPositionsData.housesPositionsData.data)
    return;
