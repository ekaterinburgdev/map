...
export function DesignCodeCardContent({ placemark }: { placemark?: DesignCodeObject }) {
  return placemark ? (
    <div className={styles.popup}>
      ...
      />
      {placemark.street && <address className={styles.popup__address}>{placemark.street}</address>}
      <Section>
        <Label color={DESIGN_CODE_MARKER_COLOR_BY_TYPE[placemark.type]}>{placemark.type}</Label>
      </Section>
      <Section>
