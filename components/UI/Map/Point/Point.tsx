...
function PointComponent({
    position, preview, isOpen, openPopup, closePopup, color, size = Sizes.M, id
}: PointProps) {
    const onClick = () => {
        if (isOpen) {
            closePopup();
    ...
    const sizeNumber = isOpen ? SIZE_BY_LETTER[size].open : SIZE_BY_LETTER[size].closed;
    const html = useMemo(() => {
        const style = `
            width:${sizeNumber}px;
        ...
        const className = classNames(styles.point, {
            ...
            [styles.point_size_s]: size === 's',
            [styles.point_size_m]: size === 'm',
        });
